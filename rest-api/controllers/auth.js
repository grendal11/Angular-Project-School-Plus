        ...
        .catch(err => {
            if (err.name === 'MongoError' && err.code === 11000) {
                ...
                res.status(409)
                    ...
                    .send({ message: `Този потребител вече съществува!` });
                return;
            }
            next(err);
        });
...
function login(req, res, next) {
    ...
    userModel.findOne({ email })
        ...
        .then(([user, match]) => {
            if (!match) {
                res.status(401)
                    .send({ message: 'Грешен email адрес или парола' });
                return
            }
...
function getProfileInfo(req, res, next) {
    ...
    .populate('schoolId') //finding by Id and returning without password and __v
